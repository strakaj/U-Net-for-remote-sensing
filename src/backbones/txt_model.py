...
class TimeTexture_flair(nn.Module):
    ...
    def __init__(self,
        ...
        encoder_name = config["encoder_name"] if "encoder_name" in config else "resnet34"
        num_ch = config['num_channels_aerial']
        self.reduction_conv = None
        if "mit" in encoder_name:
            self.reduction_conv = nn.Conv2d(num_ch, 3, 1)
            num_ch = 3
    ...
    def load_unet(self, path):
        ...
        for key, value in state_dict.items():
            if "seg_model" in key:
                new_key = key.replace("model.seg_model.", "")
                new_state_dict[new_key] = value
        self.arch_vhr.load_state_dict(new_state_dict) # strict=False

        if self.reduction_conv is not None:
            reduction_new_state_dict = OrderedDict()
            for key, value in state_dict.items():
                if "reduction_conv" in key:
                    new_key = key.replace("model.reduction_conv.", "")
                    reduction_new_state_dict[new_key] = value
            self.reduction_conv.load_state_dict(reduction_new_state_dict)
    ...
    def forward(self, config, bpatch, bspatch, dates, metadata):
        ...
        if self.reduction_conv is not None:
            bpatch = self.reduction_conv(bpatch)
