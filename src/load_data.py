...
def load_data (config: dict, val_percent=0.8): 
    ...
    val_percent = config.get("val_percent", val_percent)
    if config.get("fold", -1) >= 0:
        fold = config.get("fold", -1)
        val_percent = 1-val_percent

        idx_fold = int(np.ceil(len(trainval_domains) * val_percent))
        start_idx = idx_fold * fold
        end_idx = idx_fold * (fold+1)

        val_domains = trainval_domains[start_idx:end_idx] 
        train_domains = [*trainval_domains[0:start_idx], *trainval_domains[end_idx::]]
    else:
        idx_fold = int(len(trainval_domains) * val_percent)
        train_domains, val_domains = trainval_domains[:idx_fold], trainval_domains[idx_fold:]
